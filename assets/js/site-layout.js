...
function getWalletConnectEthereumProvider() {
  return window.WalletConnectEthereumProvider || window['@walletconnect/ethereum-provider']?.EthereumProvider || null;
}
...
async function getLiveChainId(eip1193Provider) {
  ...
  async function connectWalletConnect(options = {}) {
    const WalletConnectEthereumProvider = getWalletConnectEthereumProvider();
    if (!WalletConnectEthereumProvider) throw new Error('WalletConnect not loaded');
    ...
    const wc = await WalletConnectEthereumProvider.init({
      ...
      metadata: {
        name: window.SITE_CONFIG.siteName || 'YASHIX',
        description: 'YASHIX migration wallet connection',
        url: location.origin,
        icons: []
      }
    });
  ...
  async function restoreWalletConnectSession() {
    const WalletConnectEthereumProvider = getWalletConnectEthereumProvider();
    if (!WalletConnectEthereumProvider || typeof ethers === 'undefined') return false;
        ...
        <div class="site-shell">
          <div class="site-nav">
            <a href="index.html" class="brand" aria-label="${window.SITE_CONFIG.siteName} home">
              <img src="assets/logo.png" alt="YASHIX logo" class="brand-logo">
  ...
  root.innerHTML = `
    <footer class="site-footer site-footer-simple">
      <div class="site-shell footer-simple-wrap">
        ...
        Disclaimer: YASHIX migration participation involves risk. Verify all contract addresses before connecting or signing. Not financial advice. DYOR.
      </div>
    </footer>
  `;
}
